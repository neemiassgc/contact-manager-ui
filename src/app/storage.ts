...
export function getSelectedContact(): Contact {
	const selectedContact = localStorage.getItem("selectedContact");
	if (!selectedContact) setSelectedContact(getLocalContacts()[0]);
	return JSON.parse(localStorage.getItem("selectedContact") as string);
}
